...
from ewslf.models.util import initialize_weights
import numpy as np
...
class WSIClassifier(nn.Module):
    def __init__(self, n_classes=3, bn_track_running_stats=False, gate=True, size_arg="small", dropout=False, k_sample=8,
        ...
        if bn_track_running_stats:
            ...
            for modules in resnet.modules():
                if isinstance(modules, nn.BatchNorm2d):
                    ...
                    modules.weight = nn.Parameter(
                        torch.ones(modules.weight.shape))
                    modules.running_mean = torch.zeros(modules.weight.shape)
                    modules.bias = nn.Parameter(
                        torch.zeros(modules.weight.shape))
        ...
        self.a = nn.Sequential(nn.Linear(64, 32), nn.Tanh(), nn.Linear(32, 1))
        self.c = nn.Sequential(nn.Linear(64, 3))
        self.p = nn.Sequential(nn.Linear(64, 3))
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
        device = h.device
        h = h.view(-1, h.shape[-3], h.shape[-2], h.shape[-1])
        h = self.resnet_head(h)
        h = h.view(h.size(0), -1)
...
class EncAttn(nn.Module):
    def __init__(self, model_base):
        ...
        self.resnet_head = model_base.resnet_head
        self.attention_net = model_base.attention_net
