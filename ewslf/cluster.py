...
from ewslf.models.resnet import Enc
...
def cluster_representation(im, num_cluster=8):
    ...
    kmeans = faiss.Kmeans(img_embedding.shape[1], min(
        num_cluster, len(img_embedding)))
    ...
    with torch.no_grad():
        for im, im_list in tqdm(train_img_dic.items()):
            ...
            tdl = torch.utils.data.DataLoader(
                td, batch_size=128, shuffle=False)
